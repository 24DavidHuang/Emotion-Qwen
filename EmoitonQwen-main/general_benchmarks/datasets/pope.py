...
class PopeDataset(BaseEvalDataset):
    def __init__(
        ...
        data_path="./POPE",
        split="test",
        default_output_path="./benchmarks/logs/POPE",
        cache_dir=None,
        batch_size=64,
        sys_prompt = "Your task is to answer yes or no based on the provided question and image. Respond with yes or no.",
    ):
    ...
    def add_sys_prompt(self, question):
    ...
    def _evaluate(self, model, batch=1):
        ...
        with tqdm(total=len(self.data), desc="Evaluating") as pbar:
            for i in range(num_batches):
                ...
                batch_questions_with_sys_prompt = [self.add_sys_prompt(q) for q in batch_questions]

                # if model has batch_generate, use it
                if hasattr(model, "batch_generate") and self.batch_gen_size > 1:
                    batch_responses = model.batch_generate(batch_questions_with_sys_prompt, batch_images)
                else:
                    batch_responses = [model.generate(batch_questions_with_sys_prompt, image) for batch_questions_with_sys_prompt, image in zip(batch_questions_with_sys_prompt, batch_images)]
