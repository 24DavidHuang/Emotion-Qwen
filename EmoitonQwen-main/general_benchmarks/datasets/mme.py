...
class MMEDataset(BaseEvalDataset):
    ...
    def __init__(
        self,
        sys_prompt: str = "Your task is to answer yes or no based on the provided question and image. Respond with yes or no.",
        data_path: str = "",
        *,
        cache_dir: Union[str, None] = None,
        default_output_path: str = "./benchmarks/logs/MME",
        split: str = "test",
        debug: bool = False,
    ):
        ...
        for item in tqdm(self.data, desc="Loading data"):
            id = item["question_id"]
            print(f"Processing item with id: {id}\n")

            #新增 分类
            parts = id.split("/")
            if len(parts) > 1:
                category = parts[0].split("_")[0].lower()
                image_id = parts[-1].lower()
            else:
                raise ValueError(f"Invalid question_id format: {id}")
    ...
    def _evaluate(self, model):
            ...
            for task_name in tqdm(self.category_data[eval_type].keys(), desc=f"Evaluating {eval_type}"):
                ...
                for image_pair in tqdm(self.category_data[eval_type][task_name].values(), desc=f"Evaluating {eval_type} {task_name}"):
                    ...
                    for item in image_pair:
                        question = item["question"]
                        question = self.sys_prompt + question
                        image = item["image"]
        ...
        all_results_path = os.path.join(self.default_output_path, "MME_all_results.json")
