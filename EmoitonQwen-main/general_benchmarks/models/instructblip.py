...
class InstructBLIP(BaseModel):
    def __init__(self, model_path: str = "Salesforce/instructblip-vicuna-7b", cuda_id: int = 0, max_new_tokens=256):
        super().__init__("instructblip", model_path)
        self.device = f"cuda" if torch.cuda.is_available() else "cpu"
        self.model = InstructBlipForConditionalGeneration.from_pretrained(model_path, torch_dtype=torch.bfloat16).to(self.device)
    ...
    def generate(self, text_prompt: str, raw_image_data: str):
        ...
        generation_output = self.model.generate(**model_inputs, do_sample=False, max_length = 1024, min_length=1)
