...
def perform_inference(model, processor, text_subtitles, video_path):
    messages = [
        {
            "role": "user",
...
def evaluate_model(model, processor, csv_file, output_csv, video_folder, model_inference_func, version="CN"):
    ...
    with tqdm(total=len(df), desc="Processing EMER videos") as pbar:
        for index, row in df.iterrows():
            ...
            response_text = model_inference_func(model, processor, text_subtitles, video_path)
...
if __name__ == "__main__":
    ...
    output_csv = "/home/dell/EmotionQwen-main/emotion_benchmarks/EMER/results/Emoiton-Qwen.csv"
    ...
    evaluate_model(model, processor, csv_file, output_csv, video_folder, perform_inference, version)